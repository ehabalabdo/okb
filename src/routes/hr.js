...
router.get("/payslips/:id/pdf", async (req, res) => {
  try {
    ...
    // Only admin or the employee themselves can download payslips
...
router.patch("/clinic/location", async (req, res) => {
  ...
  try {
    ...
    // If clinic_id provided, update that clinic; otherwise update first clinic
    let query, params;
    if (clinic_id) {
      query = `UPDATE clinics SET latitude=$1, longitude=$2, allowed_radius_meters=$3,
               location_updated_at=NOW() WHERE id=$4 RETURNING *`;
      params = [latitude, longitude, radius, clinic_id];
    } else {
      query = `UPDATE clinics SET latitude=$1, longitude=$2, allowed_radius_meters=$3,
               location_updated_at=NOW() WHERE id=(SELECT id FROM clinics ORDER BY id LIMIT 1)
               RETURNING *`;
...
router.post("/employees", async (req, res) => {
  ...
  try {
    ...
    const dup = await pool.query(
      `SELECT id FROM hr_employees WHERE username=$1`,
      [username]
    );
    if (dup.rows.length) {
      return res
    ...
    const { rows } = await pool.query(
      ...
      [full_name, username, hash, phone || null, email || null, basic_salary || 0, role || 'HR_EMPLOYEE']
    );
...
router.put("/employees/:id", async (req, res) => {
  ...
  try {
    ...
    const { rows } = await pool.query(
      ...
      [full_name, phone, email, status, basic_salary, role, id]
    );
...
router.post("/me/set-pin", async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `UPDATE hr_employees SET pin_hash=$1, updated_at=NOW() WHERE id=$2`,
      [hash, hr_employee_id]
    );
...
router.post("/webauthn/register/options", async (req, res) => {
  ...
  try {
    ...
    const emp = await pool.query(
      `SELECT id, full_name, username FROM hr_employees WHERE id=$1`,
      [hr_employee_id]
    );
    if (!emp.rows.length) return res.status(404).json({ error: "Not found" });
    const e = emp.rows[0];
 ...
 */
async function validateGeoFence(latitude, longitude) {
...
router.post("/attendance/check-in", async (req, res) => {
  ...
  try {
    ...
    // Geo-fence check
    const geo = await validateGeoFence(latitude, longitude);
    if (!geo.ok) {
    ...
    const schedule = await getScheduleForDate(hr_employee_id, todayStr);
    ...
    if (existing.rows.length) {
      ...
      await pool.query(
        ...
        [hr_employee_id, todayStr, now, latitude, longitude, device_info, initialStatus]
      );
    }
...
router.get("/reports/monthly", async (req, res) => {
  try {
    const { role, type, hr_employee_id } = req.user;
  ...
  try {
    ...
    const { rows } = await pool.query(
      `SELECT * FROM hr_attendance
       WHERE employee_id=$1 AND work_date BETWEEN $2 AND $3
       ORDER BY work_date`,
      [hr_employee_id, startDate, endDate]
    );
...
router.post("/attendance/break-out", async (req, res) => {
  ...
  try {
    ...
    const att = await pool.query(
      `SELECT * FROM hr_attendance WHERE employee_id=$1 AND work_date=CURRENT_DATE`,
      [hr_employee_id]
    );
    ...
    await pool.query(
      `INSERT INTO hr_attendance_events (employee_id, work_date, event_type, event_time, latitude, longitude, device_info)
       VALUES ($1,$2,'break_out',$3,$4,$5,$6)`,
      [hr_employee_id, todayStr, now, latitude, longitude, device_info ? JSON.stringify(device_info) : null]
    );
  ...
  try {
    ...
    await pool.query(
      `UPDATE hr_attendance SET total_break_minutes = COALESCE(total_break_minutes,0) + $1,
       updated_at=NOW()
       WHERE employee_id=$2 AND work_date=CURRENT_DATE`,
      [breakMinutes, hr_employee_id]
    );
...
router.post("/deductions", async (req, res) => {
  ...
  try {
    ...
    const { rows } = await pool.query(
      ...
      [employee_id, monthDate, amount, reason || null, userId || null]
    );
...
router.delete("/deductions/:id", async (req, res) => {
  ...
  try {
    const { id } = req.params;
    const { rows } = await pool.query(
      `DELETE FROM hr_deductions WHERE id=$1 RETURNING id`,
      [id]
    );
...
router.post("/warnings", async (req, res) => {
  ...
  try {
    ...
    const { rows } = await pool.query(
      ...
      [employee_id, level, reason || null, userId || null]
    );
...
router.get("/notifications/my", async (req, res) => {
  ...
  try {
    const { hr_employee_id } = req.user;
    const { rows } = await pool.query(
      `SELECT * FROM hr_notifications WHERE employee_id=$1 ORDER BY created_at DESC`,
      [hr_employee_id]
    );
    res.json(rows.map(r => ({
      id: r.id,
...
router.patch("/notifications/:id/read", async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params;
    const { rows } = await pool.query(
      `UPDATE hr_notifications SET is_read=true WHERE id=$1 AND employee_id=$2 RETURNING id`,
      [id, hr_employee_id]
    );
    if (!rows.length) return res.status(404).json({ error: "Not found" });
    ...
    await pool.query(
      `INSERT INTO hr_social_security_settings (id, employee_rate_percent, employer_rate_percent, enabled)
       ...
       DO UPDATE SET employee_rate_percent=EXCLUDED.employee_rate_percent,
                     ...
                     updated_at=NOW()`,
      [employeeRate ?? 7.50, employerRate ?? 14.25, enabled !== false]
    );
...
router.post("/payroll/generate", async (req, res) => {
  ...
  try {
    ...
    let run = await dbClient.query(
      `SELECT * FROM hr_payroll_runs WHERE month=$1`,
      [monthDate]
    );
    ...
    if (!run.rows.length) {
      const ins = await dbClient.query(
        `INSERT INTO hr_payroll_runs (month, status, created_by) VALUES ($1,'draft',$2) RETURNING id`,
        [monthDate, userId || null]
      );
      runId = ins.rows[0].id;
    } else {
      runId = run.rows[0].id;
    ...
    const allAttendance = await dbClient.query(
      `SELECT * FROM hr_attendance WHERE work_date BETWEEN $1 AND $2`,
      [monthDate, endDate]
    );
    ...
    const allDeductions = await dbClient.query(
      `SELECT employee_id, COALESCE(SUM(amount),0) AS total FROM hr_deductions WHERE month=$1 GROUP BY employee_id`,
      [monthDate]
    );
...
router.patch("/payslips/:id", async (req, res) => {
  ...
  try {
    ...
    const current = await pool.query(
      `SELECT * FROM hr_payslips WHERE id=$1`,
      [id]
    );
...
router.post("/payroll/close-month", async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `UPDATE hr_payroll_runs SET status='closed' WHERE month=$1`,
      [monthDate]
    );
  ...
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO hr_leave_requests (employee_id, leave_type, start_date, end_date, duration_days, reason, created_at, updated_at)
       ...
       RETURNING *`,
      [hr_employee_id, leave_type, start_date, end_date, duration_days, reason || null, now]
    );
    const r = rows[0];
    res.status(201).json({
      id: r.id,
...
router.delete("/leaves/:id", async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params;
    const { rows } = await pool.query(
      `SELECT * FROM hr_leave_requests WHERE id=$1 AND employee_id=$2`,
      [id, hr_employee_id]
    );
    if (!rows.length) return res.status(404).json({ error: "Not found" });
